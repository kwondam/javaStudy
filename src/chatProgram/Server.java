	...
	void stopServer() {
		try {
			...
			while(iterator.hasNext()) {
				...
				client.socket.close();
				iterator.remove();  //모든 소켓 닫기
			}
			if(serverSocket!=null&& !serverSocket.isClosed()) {serverSocket.close();} // 서버 소켓 닫기
			if(executorService!=null&& !executorService.isShutdown()) {executorService.shutdown();} // ExecutorService 종료
			Platform.runLater(()->{
				displayText("[서버멈춤]");
				...
				try {
					while(true) {
						...
						for(Client client:connections) {
							client.send(data);  //이거뭐지?
						}
					}
				}catch(Exception e) {
					try {
						connections.remove(Client.this);
						String message = "[클리언트 통신 안됨: "+socket.getRemoteSocketAddress()+": "+Thread.currentThread().getName()+"]";
						Platform.runLater(()->displayText(message));
						socket.close();
